export const ru = {
  ...
  errors: {
    ...
    croppingImageWrong: 'Ошибка при обрезке фотографии!',
    errorWord: 'Ошибка!',
  ...
  pages: {
    ...
    forgotPassword: {
      condition: 'Если Вы не получили письмо, отправьте ссылку повторно.',
      confirmation: 'Ссылка была отправлена на почту.',
  ...
  validation: {
    ...
    onlyLetters: 'Поле должно содержать только буквы, дефис, апостроф или пробел.',
  ...
  widgets: {
    ...
    postModal: {
      ...
      confirmCloseMessage:
        'Вы действительно хотите закрыть тираж издания? При закрытии изменения не сохраняются',
